...
class UserAPI(MethodView):
    ...
    def post(self):
        # 1 获取数据
        data = request.get_json()
        # print(data)  {'username': 'zff', 'telnumber': '15626252625', 'password': '252625', 'gender': '男', 'type': ['8', '4', '2']}
        username = data.get('username')
        ...
        # 2 判断数据合法性和处理数据
        genderConfig = {'男', '女'}
        if gender and gender in genderConfig:
            if gender == '男':
                gender = 1
            else:
                gender = 2
        else:
            return jsonify(msg="请选择正确的性别", code=50000)
        ...
        return jsonify(msg="创建成功", code=20000)
    ...
    def get(self):
        ...
        current = url_for('.user', page=page, _external=True)
        prev = None
        if pagination.has_prev:
            prev = url_for('.user', page=page - 1, _external=True)
        next = None
